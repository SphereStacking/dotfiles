...
# Load icons
ICONS_PATH="$CONFIG_DIR/icons.sh"
if [ -z "$CONFIG_DIR" ]; then
  ICONS_PATH="$(dirname "$(dirname "$0")")/icons.sh"
fi
source "$ICONS_PATH" 2>/dev/null
...
launch_youtube_music() {
  ...
  sketchybar --set music label="Launching YouTube Music..." icon="$YOUTUBE_MUSIC"
  ...
  elif [ -d "/Applications/Chrome Apps.localized/YouTube Music.app" ]; then
    open -a "YouTube Music" && LAUNCHED=true
  ...
  if [ "$LAUNCHED" = true ]; then
    ...
    # Timeout
    sketchybar --set music label="YouTube Music launch timeout" icon="$YOUTUBE_MUSIC"
    (sleep 3 && "$0" display) &
  else
    echo "YouTube Music app not found in Applications folder"
    sketchybar --set music label="YouTube Music app not found" icon="$YOUTUBE_MUSIC"
    (sleep 3 && "$0" display) &
  fi
}
...
update_display() {
  ...
  if [ "$YTMD_RUNNING" = true ]; then
    ...
    sketchybar --set music label="YouTube Music not running" icon="$YOUTUBE_MUSIC" drawing=on
    exit 0
  fi
  ...
  if [ -z "$SONG_INFO" ] || ! echo "$SONG_INFO" | jq empty 2>/dev/null; then
    sketchybar --set music label="No song playing" icon="$YOUTUBE_MUSIC" drawing=on
    exit 0
  fi
  ...
  # Set play/pause icon
  if [ "$PAUSED" = "true" ]; then
    ICON="$MUSIC_PLAY"  # Play icon
  else
    ICON="$MUSIC_PAUSE"  # Pause icon
  fi
...
fast_toggle() {
  ...
  if [ -n "$CURRENT_SONG_INFO" ] && echo "$CURRENT_SONG_INFO" | jq empty 2>/dev/null; then
    ...
    if [ "$CURRENT_PAUSED" = "true" ]; then
      NEW_ICON="$MUSIC_PAUSE"  # Will become pause icon (currently playing)
    else
      NEW_ICON="$MUSIC_PLAY"  # Will become play icon (currently paused)
    fi
