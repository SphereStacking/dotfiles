...
create_music_button "music-next" "$MUSIC_NEXT" "$PLUGIN_DIR/youtube-music.sh next"

# Main music display with standardized update frequency
sketchybar --add item music right \
           --set music icon="$MUSIC_PLAY" \
                       label="Loading…" \
                       update_freq=3 \
...
create_music_button "music-prev" "$MUSIC_BACK" "$PLUGIN_DIR/youtube-music.sh previous"